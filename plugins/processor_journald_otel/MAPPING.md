...
| `_EXE` (base name)       | `Resource["process.executable.name"]`          | string  |
...
`process.executable.name` is derived: it is the base name of
`process.executable.path`, that is of the target of `/proc/[pid]/exe`. As described in the semantic convention.

Fields listed as `int` are converted from their journald string form. A value
that does not parse as an integer is not stored under its semantic convention
name, so a typed attribute never holds a wrong type. It is kept under its
`journald.` prefixed field name instead, so no data is lost on a malformed
entry.
