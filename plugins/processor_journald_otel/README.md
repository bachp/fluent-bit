...
- Fields with no mapping keep their journald name under a `journald.` prefix
  and their original value, so nothing from the journal is lost. A field whose
  value does not fit the type of the semantic convention it maps to is kept
  the same way, rather than giving a typed attribute a wrong type.
