...
static const struct jo_field jo_resource_attributes[] = {
    ...
    {"_COMM",     "process.executable.name", FLB_FALSE},
 ...
 * which case the caller keeps the original string.
 */
                            ...
                            struct cfl_variant *var)
{
    ...
    if (field->numeric == FLB_TRUE && var->type == CFL_VARIANT_STRING &&
        jo_parse_int64(var->data.as_string, cfl_variant_size_get(var),
                       &number) == FLB_TRUE) {
        return cfl_kvlist_insert_int64_s(target, (char *) field->otel,
                                         strlen(field->otel), number);
    }

    /* not a number after all, keep the original value */
                         ...
                         struct cfl_kvlist **out_resource)
{
    ...
    cfl_list_foreach(head, &body->list) {
        ...
        if (field) {
            if (jo_insert_mapped(attributes, field, kvpair->val) != 0) {
                goto failure;
            }
            continue;
        }
        ...
        if (field) {
            if (jo_insert_mapped(resource, field, kvpair->val) != 0) {
                goto failure;
            }
            continue;
        }
